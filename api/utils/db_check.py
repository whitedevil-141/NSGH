...
def _simplify_type(t: TypeEngine) -> str:
    ...
    def _model_col_to_sql(col, dialect_name: str) -> str:
        ...
        This is best-effort: String -> VARCHAR(length) or VARCHAR(255),
    ...
    def check_schema(engine, base, apply: bool = False) -> List[Tuple]:
        ...
        Returns a report list similar to previous format.
        """
        ...
        for table_name, table in model_tables.items():
            ...
            for col_name, col in model_cols.items():
                ...
                # type comparison (best-effort): treat common pairs as equivalent
        ...
        # Execute safe fixes: only add columns and adjust nullability
            ...
            for f in fixes:
                try:
                    if f[0] == "add_column":
                        ...
                        col_name = f[2]
                        make_nullable = f[3]
                        # For MySQL use MODIFY, for Postgres use ALTER COLUMN ... SET/DROP NOT NULL
                        if dialect == 'mysql':
                            # We need column type to MODIFY; fetch column definition
